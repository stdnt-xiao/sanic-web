...
export const comma = (num: any, suffix = ''***REMOVED*** => {
  ...
  ***REMOVED***
  ...
  if (strNum.includes('.'***REMOVED******REMOVED*** {
    ...
    source[0] = source[0].replace(/(\d***REMOVED***(?=(\d{3***REMOVED******REMOVED***+$***REMOVED***/ig, '$1,'***REMOVED***// 只将整数部分进行都好分割
    return source.join('.'***REMOVED*** + suffix // 再将小数部分合并进来
  ***REMOVED***

  return strNum.replace(/(\d***REMOVED***(?=(\d{3***REMOVED******REMOVED***+$***REMOVED***/ig, '$1,'***REMOVED*** + suffix
***REMOVED***
