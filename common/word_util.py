...
class WordUtil:
    ...
    @classmethod
    def read_target_content(cls, file_key, target_heading_text="详细功能需求"***REMOVED***:
        """
        读取位于指定一级标题下的所有内容。
        ...
        for para in document.paragraphs:
            # 检查是否为一级标题
            if para.style.name.lower(***REMOVED***.startswith("heading 1"***REMOVED***:
            ...
            # 只有在目标部分内时才收集内容
            if in_target_section:
                if para.style.name.lower(***REMOVED***.startswith("heading"***REMOVED***:  # 如果是任何级别的标题
                    if current_heading is not None:
                        outline_items.append((current_heading, content_for_current_heading***REMOVED******REMOVED***
                    current_heading = para.text
                    content_for_current_heading = []
                else:
                    if current_heading is not None:
                        content_for_current_heading.append(para.text***REMOVED***
