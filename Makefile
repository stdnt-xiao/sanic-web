...
# 服务端 Docker 镜像标签
SERVER_DOCKER_IMAGE = apconw/$(SERVER_PROJECT_NAME):1.1.8
...
docker-build-aliyun-server-multi:
	docker buildx build --platform linux/amd64,linux/arm64 --push -t $(ALIYUN_IMAGE_NAME):1.1.8 -f ./docker/Dockerfile .
